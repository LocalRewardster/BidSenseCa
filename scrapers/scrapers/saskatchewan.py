...
class SaskatchewanScraper(BaseScraper):
    """Scraper for Saskatchewan tender opportunities."""
    
    def __init__(self):
        super().__init__(source_name="saskatchewan")
        self.name = "saskatchewan"
        self.base_url = "https://saskatchewan.bidsandtenders.ca"
        self.search_url = "https://saskatchewan.bidsandtenders.ca/Module/Tenders/en/Search"
    ...
    async def scrape_tenders(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Scrape tenders from Saskatchewan."""
        try:
            # Get search page
            ...
            logger.error(f"Error scraping Saskatchewan: {e}")
            return []


async def main():
    """Main function for running Saskatchewan scraper directly."""
    import asyncio
    
    # Configure logging
    logger.add(
        "logs/saskatchewan.log",
        rotation="1 day",
        retention="7 days",
        level="INFO"
    )
    
    async with SaskatchewanScraper() as scraper:
        count = await scraper.run(limit=10)
        print(f"Saskatchewan: {count} tenders saved")
