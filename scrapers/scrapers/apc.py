...
class APCScraper(BaseScraper):
    """Scraper for Alberta Purchasing Connection tender opportunities."""
    
    def __init__(self):
        super().__init__(source_name="apc")
        self.name = "apc"
        self.base_url = "https://alberta.bidsandtenders.ca"
        self.search_url = "https://alberta.bidsandtenders.ca/Module/Tenders/en/Search"
    ...
    def parse_search_results(self, soup: BeautifulSoup) -> List[Dict]:
        ...
        # Look for tender items in search results
        ...
        for item in tender_items:
            try:
                ...
                # Extract metadata
                meta_div = item.find('div', class_='tender-meta')
                reference = ""
                organization = ""
                closing_date = ""
                ...
                # Extract description
                desc_div = item.find('div', class_='tender-description')
    ...
    def parse_tender_details(self, soup: BeautifulSoup, url: str) -> Dict:
        ...
        try:
            ...
            # Extract information from tender-info section
            info_div = soup.find('div', class_='tender-info')
            if info_div:
                # Reference
    ...
    async def scrape_tenders(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """Scrape tenders from Alberta Purchasing Connection."""
        try:
            # Get search page
            ...
            logger.error(f"Error scraping Alberta Purchasing Connection: {e}")
            return []


async def main():
    """Main function for running APC scraper directly."""
    import asyncio
    ...
    async with APCScraper() as scraper:
        count = await scraper.run(limit=10)
        print(f"Alberta Purchasing Connection: {count} tenders saved")
