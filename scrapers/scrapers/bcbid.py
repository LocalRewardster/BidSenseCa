...
class BCBidScraper(BaseScraper):
    """Scraper for BC Bid tender opportunities."""
    
    def __init__(self):
        super().__init__(source_name="bcbid")
        self.name = "bcbid"
        self.base_url = "https://bcbid.gov.bc.ca"
        self.search_url = "https://bcbid.gov.bc.ca/opportunities"
    ...
    async def scrape_tenders(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"Error scraping BC Bid: {e}")
            return []


async def main():
    """Main function for running BC Bid scraper directly."""
    import asyncio
    ...
    async with BCBidScraper() as scraper:
        ...
        print(f"BC Bid: {count} tenders saved")
