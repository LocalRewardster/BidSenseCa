...
class ManitobaScraper(BaseScraper):
    """Scraper for Manitoba tender opportunities."""
    
    def __init__(self):
        super().__init__(source_name="manitoba")
        self.name = "manitoba"
        self.base_url = "https://manitoba.bidsandtenders.ca"
        self.search_url = "https://manitoba.bidsandtenders.ca/Module/Tenders/en/Search"
    ...
    async def scrape_tenders(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"Error scraping Manitoba: {e}")
            return []


async def main():
    """Main function for running Manitoba scraper directly."""
    import asyncio
    ...
    async with ManitobaScraper() as scraper:
        count = await scraper.run(limit=10)
        print(f"Manitoba: {count} tenders saved")
